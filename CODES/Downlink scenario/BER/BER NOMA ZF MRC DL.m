...
%%%other antenna will send  signal for user2 (x2),,,,, near reciver U(1) has 2 antennas ,, so 
...
% % % % %     err4mmse=[];
    ...
    err6zf=[];
     for j=1:1:epoc
        %%%%%%%%%%%%large scale fading
        l1=db2pow(31+10*eta*log10(d1)-G1+8*randn(1,1)); % path budgate with shadowing
        l2=db2pow(31+10*eta*log10(d2)-G2+8*randn(1,1)); %path 2 budgate with shadowing
        P1=db2pow(10*log10(a1)+p);
        P2 =db2pow(10*log10(a2)+p);
        int1=db2pow(10*log10(a2)+p); %power user2 at us1
        int2=db2pow(10*log10(a1)+p); %power us1 at us2
        %%%%%%%%%%%%%%%%%%%%%%%%snall scale fading
        ...
        y1=[sqrt(P1)*h1p.*x1+sqrt(int1)*h1i.*x2+noise];
        noise=sqrt(n)*randn(2,num)/sqrt(2);
        y2=[sqrt(int2)*h2i.*x1+sqrt(P2)*h2p.*x2+noise];
        %%%%%%55mmmrrrccc
        wmrc11=h1p/norm(h1p)^2;wmrc11=wmrc11';
        wmrc12=h1i/norm(h1i)^2;wmrc12=wmrc12';
        %%%%%%%%%sic
        m2at1mrc=pskdemod(real(wmrc12*y1),2);
        x21mrc=pskmod(m2at1mrc,2);
        ynew1mrc=y1-sqrt(int1)*h1i*x21mrc;
        m1mrc=pskdemod(real(wmrc11*ynew1mrc),2);
        %%%555
        wmrc2=h2p/norm(h2p)^2;wmrc2=wmrc2';
