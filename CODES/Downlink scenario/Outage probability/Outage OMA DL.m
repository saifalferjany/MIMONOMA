...
%%OMA system
%THE SCENARIO:
%A MIMO(2-2) WITH 2 USERS SYSTEM, BS SEND A MASSAGE BY THE HALF BW AND 
%THE SAME POWER POWER TO EACH USER ... SIMPLY THE OUTAGE IS THE PERCENTAGE THAT
...
P1 = 10*log10(a1)+p+l1+G1; 
P2 = 10*log10(a2)+p+l2+G2;
P1=db2pow(P1);
P2=db2pow(P2);
...
p=pt+30;
semilogy(p,po,'b-','LineWidth',1,'DisplayName','near user outage probilty in OMA d=100');
hold on
semilogy(p,po2,'r+','LineWidth',1,'DisplayName','far user outage probilty in OMA d=500');
legend('show')
xlim([min(p) max(p)])
